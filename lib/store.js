...
function getUser(userId) {
  ...
  if (!usersData[userId]) usersData[userId] = { pushType: '', pushKey: '', endedAlert: false, subs: [], weiboSubs: [] };
  if (typeof usersData[userId].endedAlert !== 'boolean') usersData[userId].endedAlert = false;
  if (!Array.isArray(usersData[userId].subs)) usersData[userId].subs = [];
  if (!Array.isArray(usersData[userId].weiboSubs)) usersData[userId].weiboSubs = [];
...
function getPreferences(userId) {
  const u = getUser(userId);
  if (!u) return { endedAlert: false };
  return { endedAlert: !!u.endedAlert };
}

function setPreferences(userId, prefs) {
  const u = getUser(userId);
  if (!u) return false;
  if (prefs && typeof prefs.endedAlert === 'boolean') u.endedAlert = prefs.endedAlert;
  save();
  return true;
}
...
// ===== 微博订阅 =====
function getWeiboSubs(userId) {
  const u = getUser(userId);
  return u ? (u.weiboSubs || []) : [];
}
function addWeiboSub(userId, sub) {
  const u = getUser(userId);
  if (!u) return false;
  if (!Array.isArray(u.weiboSubs)) u.weiboSubs = [];
  u.weiboSubs.push(sub);
  save();
  return true;
}
function removeWeiboSub(userId, subId) {
  const u = getUser(userId);
  if (!u || !Array.isArray(u.weiboSubs)) return false;
  const idx = u.weiboSubs.findIndex(function (s) { return s.id === subId; });
  if (idx < 0) return false;
  u.weiboSubs.splice(idx, 1);
  save();
  return true;
}
function updateWeiboSub(userId, subId, updates) {
  const u = getUser(userId);
  if (!u || !Array.isArray(u.weiboSubs)) return false;
  const sub = u.weiboSubs.find(function (s) { return s.id === subId; });
  if (!sub) return false;
  Object.assign(sub, updates);
  save();
  return true;
}
// 按 uid 去重,返回 [{ uid, owners: [{ userId, sub }] }]
function getAllWeiboSubsGroupedByUid() {
  const map = {};
  for (const userId in usersData) {
    const u = usersData[userId];
    if (!u || !Array.isArray(u.weiboSubs)) continue;
    for (const sub of u.weiboSubs) {
      if (!sub || !sub.uid) continue;
      const key = 'weibo:' + sub.uid;
      if (!map[key]) map[key] = { uid: String(sub.uid), owners: [] };
      map[key].owners.push({ userId: userId, sub: sub });
    }
  }
  return Object.values(map);
...
function cleanupEmptyUsers() {
  ...
  for (const userId in usersData) {
    const u = usersData[userId];
    const noLive = !u.subs || u.subs.length === 0;
    const noWeibo = !u.weiboSubs || u.weiboSubs.length === 0;
    if (noLive && noWeibo && !u.pushType && !u.pushKey) {
...
module.exports = {
  ...
  getWeiboSubs: getWeiboSubs,
  addWeiboSub: addWeiboSub,
  removeWeiboSub: removeWeiboSub,
  updateWeiboSub: updateWeiboSub,
  getAllWeiboSubsGroupedByUid: getAllWeiboSubsGroupedByUid,
