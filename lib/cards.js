...
// 激活卡密（绑定到用户）
// 支持已使用卡密的"恢复/重新绑定":只要没过期、没停用,用户重新输入卡密即可绑定到新的 userId
// 返回字段: rebound=true 表示是重新绑定(原用户数据需迁移), alreadyBound=true 表示已绑到当前用户
function activateCard(code, userId) {
  ...
  // 已使用的卡密:允许重新绑定到新用户(前提是没过期)
  if (card.status === 'used') {
    const exp = new Date(card.expiresAt).getTime();
    if (exp <= now.getTime()) {
      return { success: false, error: '卡密已过期,无法恢复,请联系管理员' };
    }
    // 已绑定到当前用户:无需重复操作
    if (card.boundUserId === userId) {
      return { success: true, card: card, rebound: false, alreadyBound: true };
    }
    // 重新绑定到新用户
    const oldUserId = card.boundUserId;
    card.boundUserId = userId;
    save();
    return { success: true, card: card, rebound: true, oldUserId: oldUserId };
  }
...
// ===== 卡密上的推送配置(随卡密走,换 userId 也能恢复) =====
function getCardPushConfig(code) {
  const card = cardsData.cards[code];
  if (!card) return { pushType: '', pushKey: '' };
  return { pushType: card.pushType || '', pushKey: card.pushKey || '' };
}

function setCardPushConfig(code, cfg) {
  const card = cardsData.cards[code];
  if (!card) return false;
  card.pushType = (cfg && cfg.pushType) || '';
  card.pushKey = (cfg && cfg.pushKey) || '';
  save();
  return true;
}
...
// 优先级: 未过期的正式卡密 > 未过期的试用卡密 > 已过期的卡密(显示过期)
// 注意:只有试用卡密的用户会被限制 maxLiveSubs=1,有正式卡密则用正式卡密配额
function getUserAuth(userId) {
  var now = Date.now();
  var activeFormal = null;      // 未过期的正式卡密
  var activeTrial = null;       // 未过期的试用卡密
  var latestExpired = null;     // 已过期的卡密(取最近过期的)
  for (const code in cardsData.cards) {
    const card = cardsData.cards[code];
    if (card.boundUserId === userId && card.status === 'used') {
      ...
      if (active) {
        if (card.type === 'trial') {
          if (!activeTrial || exp > new Date(activeTrial.expiresAt).getTime()) activeTrial = { card: card, code: code };
        } else {
          if (!activeFormal || exp > new Date(activeFormal.expiresAt).getTime()) activeFormal = { card: card, code: code };
        }
      } else {
        if (!latestExpired || exp > new Date(latestExpired.expiresAt).getTime()) latestExpired = { card: card, code: code };
      }
    }
  }
  // 选卡密:正式卡密优先
  var pick = activeFormal || activeTrial || latestExpired;
  if (pick) {
...
module.exports = {
  ...
  getCardPushConfig: getCardPushConfig,
  setCardPushConfig: setCardPushConfig
};
