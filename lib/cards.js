...
function getUserAuth(userId) {
  // 找绑定到这个用户的所有卡密,优先选有效期最长且未过期的
  // 试用卡(maxLiveSubs=1)要优先返回,防止用试用身份绕过订阅限制
  var best = null;
  var bestCode = null;
  ...
  for (const code in cardsData.cards) {
    ...
    if (card.boundUserId === userId && card.status === 'used') {
      ...
      // 非Trial卡:选有效期最长的那张
      if (!best || exp > new Date(best.expiresAt).getTime()) {
        best = card;
        bestCode = code;
        bestActive = active;
      }
  ...
  if (best) {
    const now = Date.now();
    const exp = new Date(best.expiresAt).getTime();
    const daysLeft = Math.max(0, Math.ceil((exp - now) / (24 * 3600 * 1000)));
    return {
      active: bestActive,
      expiresAt: best.expiresAt,
      maxLiveSubs: best.maxLiveSubs,
      cardCode: bestCode,
      daysLeft: daysLeft,
      type: best.type || 'formal',
      expiredTrial: !bestActive && best.type === 'trial'
    };
  }
